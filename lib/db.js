...
Database.prototype.addReminder = function (reminder, cb) {
    this.db.collection('reminders').insert(reminder, { safe: true }, cb); 
};

Database.prototype.removeReminder = function (reminder) {
    this.db.collection('reminders').remove(reminder, {justOne: true}, function () {});
};

Database.prototype.getReminders = function (cb) {
    this.db.collection('reminders').find().toArray(cb);
};
...
Database.prototype.getQuoteCount = function(cb){
    this.db.collection('quotes').stats(cb);
};

Database.prototype.addNewQuote = function(quote, cb){
    var that = this;
    this.makeIdFor('quotes', function(err, rec){
        if(err){
            console.warn(err.message);
        } else {
            that.db.collection('quotes').insert({ _id: rec.count, quote: quote }, function(err, res){
                cb(err, rec.count);
            });
        }
    });
...
Database.prototype.seenLink = function(link, cb) {
    this.db.collection('seenlinks').find({ url: link }).toArray(cb);
};

Database.prototype.addSeenLink = function (link, sender, cb) {
    this.db.collection('seenlinks').insert({ url: link, sender: sender, sentAt: Date.now() }, { safe: true, w: 0 });
};
