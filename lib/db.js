...
Database.prototype.getIdFor = function(name){
    this.db.collection('idcounters').findAndModify({ query: { _id: name }, update: { $inc: { count: 1 } }, new: true }, function(err, rec){
        if(err){
...
Database.prototype.addNewQuote = function(quote){
    this.db.collection('quotes').insert({ _id: this.getIdFor('quotes'), quote: quote });
};
